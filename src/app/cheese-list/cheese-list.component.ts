...
export class CheeseListComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.Cheese.getAllCheeses().subscribe((data: any) => {
      this.cheeses = data;
    });
  }

  setCheeses(){
    this.Cheese.getCheesesBy(this.milk, this.dpt).subscribe((data: any) => {
      this.cheeses = data;
    });
  }

  onMilkChange(e: any) {
    this.milk = e.target.value;
    this.setCheeses();
  }

  onDptChange(e: any) {
    this.dpt = e.target.value;
    this.setCheeses();
  }
}
